...
class ServerCrypto:
    def __init__(self, certificate_file, ca_public_key_file):
        ...
        print(f"Server using signature algorithm: {self.signature_algorithm}")
        print(f"Server entity ID: {cert_data['entity_id']}")
    
    def create_session_keypair(self):
        """Tạo cặp khóa ECDH cho session"""
    ...
    def perform_ecdh_with_reader(self, server_private_key, reader_public_key_b64):
        """Thực hiện ECDH với reader"""
        ...
        shared_key = server_private_key.exchange(ec.ECDH(), reader_public_key)
